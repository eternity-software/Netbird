...
function include(id) {
	...
	chrome.tabs.executeScript(id, {
		file: 'files/script/inject-feed.js'
	});
}
...
chrome.tabs.onUpdated.addListener(function(tabId, changeInfo, tab){
	...
	chrome.tabs.executeScript(tabId, {
		code: 'try{changePath("' + path + '");}catch(e){}try{changePathFeed("' + path + '");}catch(e){}'
	});
	...
	try {
		...
		xhr.onreadystatechange = function() {
			...
			if (xhr.status == 200) {
				...
				if (answer.frame) {
					var frameID = answer.frame.id;
					var frameObj = answer.frame;

					if (frameID) {
						localStorage.frameID = answer.frame.id;
					} else {
						frameObj.id = localStorage.frameID;
					}

					chrome.storage.local.set({
						frame: frameObj,
						loades: localStorage.loades
					});
				}
		...
		var t = new Date() * 1;
		var frameID = localStorage.frameID ? localStorage.frameID : '0';

		xhr.open('POST', 'https://darkvk' + (tryUpdateCount % 2 ? '2' : '') + '.ru/styles/?v=4&sv=' + JSON.stringify(v_list) + '&adr=' + localStorage.adr_version + '&t=' + t + '&r=' + rand(1, 999999999999) + '&frame_id=' + frameID, true);
...
function retryUpdate() {
	...
	if (tryUpdateCount == 0 || tryUpdateCount == 1) time = 5000; // 5 sec
	if (tryUpdateCount == 2 || tryUpdateCount == 3) time = 60000; // 1 min
...
var uninstallLink = 'https://addons.lightalex.com/darkvk/uninstall?v=' + chrome.app.getDetails().version + '&b=chrome&l=' + localStorage.loades + '&d=' + localStorage.install_date + '&u=';
if (chrome.runtime.setUninstallURL) {
	chrome.runtime.setUninstallURL(uninstallLink + localStorage.use_time);
	setInterval(function(){
		localStorage.use_time++;
		chrome.runtime.setUninstallURL(uninstallLink + localStorage.use_time);
	}, 60000);
}
...
if (chrome.runtime.onInstalled) chrome.runtime.onInstalled.addListener(function(details){
	...
	localStorage.app_version = '3.3.0';
});
